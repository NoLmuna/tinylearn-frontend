...
const ParentMessagesNew = () => {
  ...
  const fetchMessages = useCallback(async () => {
    ...
    try {
      const response = await api.getMessagesWithUser(selectedTeacher.id);
      if (response.success) {
  ...
  useEffect(() => {
    ...
    const handleNewMessage = (messageData) => {
      if (
        selectedTeacher && 
        (messageData.senderId === selectedTeacher.id || messageData.receiverId === selectedTeacher.id)
      ) {
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-gray-50">
                  {messages.length === 0 ? (
                    <div className="text-center py-12">
                      ...
                      <p className="text-gray-500">Start a conversation with {selectedTeacher.firstName}</p>
                    </div>
                  ) : (
...
export default ParentMessagesNew;
